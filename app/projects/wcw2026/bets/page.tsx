  ...
  const renderTable = (bs: any[], person: string) => (
    <div className="mb-8">
      <div className="flex items-center gap-3 mb-4">
        <span>👤</span>
        <span className="font-semibold text-[#f1f1ef]">{person}</span>
        <span className="text-xs text-[#5e5d59] font-mono">{bs.length}注 · {bs.reduce((s,b) => s+b.stake, 0)}元</span>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {bs.map(b => {
              ...
              return <tr key={b.id} className="border-b border-[#1f1e1d]/50 hover:bg-white/[0.02] transition-colors">
                <td className="py-3 text-xs text-[#5e5d59] font-mono">{m?.match_date?.slice(5)}</td>
                <td className="py-3 text-sm text-[#b0aea5]">{m ? `${m.home_team} vs ${m.away_team}` : '-'}</td>
                <td className="py-3"><span className={`tag ${b.direction === 'H' ? 'tag-h' : b.direction === 'A' ? 'tag-a' : 'tag-d'}`}>{b.direction === 'H' ? '主胜' : b.direction === 'A' ? '客胜' : '平'}</span></td>
                <td className="py-3 text-[#b0aea5] font-mono">{b.odds}</td>
                <td className="py-3 text-[#b0aea5] font-mono">{b.stake}元</td>
                <td className="py-3"><span className={`tag ${b.result === 'won' ? 'tag-win' : b.result === 'lost' ? 'tag-lose' : 'tag-pending'}`}>{b.result === 'won' ? '✅ 赢' : b.result === 'lost' ? '❌ 输' : '⏳'}</span></td>
                <td className={`py-3 font-mono font-medium ${b.profit > 0 ? 'text-[#4ade80]' : b.profit < 0 ? 'text-[#f87171]' : 'text-[#5e5d59]'}`}>{b.result === 'pending' ? '-' : `${b.profit > 0 ? '+' : ''}${b.profit}`}</td>
              </tr>
            })}
          </tbody>
        </table>
