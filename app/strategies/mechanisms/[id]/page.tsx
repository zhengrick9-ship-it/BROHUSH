...
export default async function MechanismDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <ResearchShell eyebrow={`STRATEGY LIBRARY / MECHANISM / ${item.id}`} title={item.name} description={item.purpose}>
    ...
    <section className="mt-4 dashboard-panel"><div className="panel-heading"><div><p className="section-label">CURRENT RUN</p><h2>2026-08-31 收盘结果</h2></div><a href="/strategies/outputs/20260831T224529-low-cross-window-v1" className="text-xs font-semibold text-[var(--accent-dark)]">查看完整输出 →</a></div><p className="mt-4 text-sm leading-7 text-[var(--secondary)]">{item.result}</p></section>
    <section className="mt-4 dashboard-panel"><div className="panel-heading"><div><p className="section-label">SOURCE</p><h2>源文件</h2></div></div><div className="mt-4 grid gap-2 text-sm"><a className="border-t border-[var(--line)] pt-3 text-[var(--accent-dark)]" href={`/strategy-source/low-cross-window-v1/strategies/${item.id}/config.json`}>本套策略 config.json →</a><a className="border-t border-[var(--line)] pt-3 text-[var(--accent-dark)]" href="/strategy-source/low-cross-window-v1/registry.json">完整 registry.json →</a><a className="border-t border-[var(--line)] pt-3 text-[var(--accent-dark)]" href="/strategy-source/low-cross-window-v1/run_walkforward.py">回测源代码 run_walkforward.py →</a><a className="border-t border-[var(--line)] pt-3 text-[var(--accent-dark)]" href="/strategy-source/low-cross-window-v1/README.md">说明文档 README.md →</a></div></section>
  </ResearchShell>;
}
