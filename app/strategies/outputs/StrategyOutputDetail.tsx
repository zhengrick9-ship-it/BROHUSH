...
export function StrategyOutputDetail({ output, isLatest, archived = false }: { output: StrategyOutput; isLatest: boolean; archived?: boolean }) {
  ...
  return <ResearchShell eyebrow="STRATEGY LIBRARY / OUTPUT DETAIL" title={`${output.strategy ?? "策略输出"} · ${output.versionLabel ?? output.version}`} description="单个版本的完整结果、筛选边界与人工分析。历史版本保留，当前版本单独标记。">
    ...
    <section className="metric-grid metric-grid-wide mt-4">
      ...
      <div className="metric"><p className="section-label">可执行行业交集</p><p className="mt-3 text-xl font-semibold">{output.industryCoherentGates ?? output.industryGates ?? 0}</p><p className="mt-2 text-xs text-[var(--muted)]">至少含1只严格候选</p></div>
      <div className="metric"><p className="section-label">可执行主题交集</p><p className="mt-3 text-xl font-semibold">{output.themeCoherentGates ?? output.themeGates ?? 0}</p><p className="mt-2 text-xs text-[var(--muted)]">至少含1只严格候选</p></div>
    </section>
    ...
    <section className="mt-8 dashboard-panel"><div className="panel-heading"><div><p className="section-label">03 / 板块共振</p><h2>申万二级低位行业</h2></div><p>只展示与严格个股候选相交者</p></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><thead className="text-[var(--muted)]"><tr><th className="py-3 pr-4">排名</th><th className="py-3 pr-4">行业</th><th className="py-3 pr-4">评分</th><th className="py-3 pr-4">严格候选数</th><th className="py-3 pr-4">低位比例</th><th className="py-3 pr-4">中位位置</th><th className="py-3">MACD有利比例</th></tr></thead><tbody>{(output.industries ?? []).map((item: any, index: number) => <tr className="border-t border-[var(--line)]" key={item.name}><td className="py-3 pr-4">{index + 1}</td><td className="py-3 pr-4 font-semibold">{item.name}</td><td className="py-3 pr-4">{Number(item.score ?? 0).toFixed(2)}</td><td className="py-3 pr-4">{item.mechanicalHitCount ?? "—"}</td><td className="py-3 pr-4">{ratio(item.lowRatio)}</td><td className="py-3 pr-4">{pct(item.medianPos60)}</td><td className="py-3">{ratio(item.macdRatio)}</td></tr>)}</tbody></table></div></section>

    <section className="mt-4 dashboard-panel"><div className="panel-heading"><div><p className="section-label">04 / 主题研究映射</p><h2>同花顺主题低位榜</h2></div><p>只展示与严格个股候选相交者 · 映射截至 {output.mappingAsOf}</p></div><div className="mt-4 grid gap-2 md:grid-cols-2">{(output.themes ?? []).map((item: any, index: number) => <div className="flex items-center justify-between gap-3 border-t border-[var(--line)] pt-3" key={item.name}><div><p className="text-sm font-semibold">{index + 1}. {item.name}</p><p className="mt-1 text-xs text-[var(--muted)]">严格候选 {item.mechanicalHitCount ?? "—"} · 低位 {ratio(item.lowRatio)} · 中位位置 {pct(item.medianPos60)} · 正流 {ratio(item.flowRatio)}</p></div><p className="text-sm font-semibold">{Number(item.score ?? 0).toFixed(2)}</p></div>)}</div></section>
